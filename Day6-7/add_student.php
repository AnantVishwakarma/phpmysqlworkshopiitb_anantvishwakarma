...
if(isset($_POST['sname']) && isset($_POST['email']) && isset($_POST['password']))
{
    ...
    {
        $stmt = $conn->prepare("INSERT INTO student (`sname`,`email`,`password`) VALUES (?,?,?);");
    ...
    else
    {
        ...
        $mysql=$_POST['mysql'];
        $html=$_POST['html'];
        $stmt = $conn->prepare("INSERT INTO student (`sname`,`email`,`password`,`php`,`mysql`,`html`) VALUES (?,?,?,?,?,?);");
        $stmt->bind_param("sssiii",$sname,$email,$password,$php,$mysql,$html);
    }
    
    if($stmt->execute())
    {
        echo "Record Added Successfully<br><a href='admin_dashboard.php'>Click here</a> to go back to Dashboard.";
    }
    else
    {
        die("Error Occured: " . $stmt->error . "<br><a href='admin_dashboard.php'>Click here</a> to go back to Dashboard.");
    }    
    $stmt->close();
    $conn->close();
}
